<?php
namespace website_project\utilities;
...
class Validate {
    ...
    public function contentCheck() {
        ...
        $this->emailAddressCheck($this->data['email']);
        $this->passwordCheck($this->data['password']);
        $this->emailCheck($this->data['email']);
        return $this->validate();
    }

    protected function emailAddressCheck($email) {
        $db = DB::getInstance();
        $pdo = $db->getConnection();
        try {
            $this->query = "SELECT 1 FROM mysimpleregistration WHERE email = :email";
